  ...
  return (
    <div className="grid items-center gap-10 sm:grid-cols-2 sm:gap-14">
      ...
      >
        ...
        <div className="mt-1.5 font-mono text-[0.75rem] tracking-[0.08em] text-text-faint">
          {t("macrosLabel")}
        </div>
        <div className="flex flex-wrap gap-1.5">
          {macros.map((macro, i) => (
            <span
              key={i}
              className="rounded-md border border-dashed border-line-strong bg-surface-panel px-3 py-2 font-mono text-[0.78125rem] text-text-muted"
            >
              {macro}
            </span>
          ))}
