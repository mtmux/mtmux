...
export type RelayRuntime = {
  ...
  /**
   * Arms local pairing — the QR's nonce and the digits under it — and returns
   * the nonce to encode.
   *
   * The code is generated by the CLI and handed in, not minted here: the
   * format belongs to `@repo/crypto`, and the relay holding a second opinion
   * about how many digits a local code has is exactly the drift worth
   * preventing.
   */
  armLocalPairing: (
    code: string,
   ...
   *
   * All three outcomes, not only success. A code somebody guessed at five
   * times is as dead as one that was used, and a terminal still showing it is
   * a terminal lying to its owner.
   */
  onLocalPairingSpent: (
    listener: (outcome: "paired" | "refused" | "burned") => void,
  ) => () => void;
  /**
   * The question asked at the machine before a local device is let in.
   *
   * Optional, and the relay treats "no gate installed" as an approval — see
   * `pairing-local.ts` for why that is the safe reading rather than the unsafe
   * one. This CLI always installs it.
   */
  setLocalPairingGate?: (
    gate:
